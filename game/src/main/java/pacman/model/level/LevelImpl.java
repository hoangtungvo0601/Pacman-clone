...
public class LevelImpl implements Level {
    ...
    private void initLevel(LevelConfigurationReader levelConfigurationReader) {
        ...
        Map<GhostMode, Integer> modeLengths = levelConfigurationReader.getGhostModeLengths();

        for (Ghost ghost : this.ghosts) {
            player.registerFrightenedObserver(ghost);
            player.registerObserver(ghost);
            ghost.setSpeeds(ghostSpeeds);
            ghost.setLength(modeLengths);
            ghost.setGhostMode(GhostMode.SCATTER);
        }

        for(Ghost ghostA : this.ghosts){
            for(Ghost ghostB : this.ghosts)if(!ghostA.equals(ghostB)){
                ghostA.registerGhostPositionObserver(ghostB);
            }
        }
    ...
    public void tick() {
        if (this.gameState != GameState.IN_PROGRESS) {
            ...
            for(Ghost ghost : this.ghosts){
                ghost.notifyOtherGhostObservers();
            }
        }
    ...
    public void ghostEaten(){
        int pointEarned = this.player.consecutiveEatenGhost();
        this.points += pointEarned;
        notifyObserversWithScoreChange(pointEarned);
    }
}
